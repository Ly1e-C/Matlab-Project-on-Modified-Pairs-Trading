function varargout = pairs3(series2, M, N, spread, scaling, cost)
...
for i = max(M,N)+1 : N : length(s)-N
    ...
    if h ~= 0
        ...
        res(i:i+N-1) = series2(i-1:i+N-2, 1) ...
            - (reg1.coeff(1) + reg1.coeff(2).*series2(i-1:i+N-2, 2));
        reg0(i:i+N-1, 1) = reg1.coeff(1);
        reg0(i:i+N-1, 2) = reg1.coeff(2);
        reg0(i:i+N-1, 3) = reg1.RMSE;
        ...
        % opposite.
        indicate(i:i+N-1) = res(i:i+N-1)/reg1.RMSE;
        
        s(i:i+N-1, 2) = (indicate(i:i+N-1) > spread) ...
            - (indicate(i:i+N-1) < -spread);
        s(i:i+N-1, 1) = -1/reg1.coeff(2) .* s(i:i+N-1, 2);
    end
end

%% Calculate performance statistics

pxChange = [0 0; diff(series2)];
sChange = [0 0; diff(s)];
r0 = [s.* pxChange - abs(sChange)*cost/2];
r  = sum(r0 ,2);
sh = scaling*sharpe(r,0); 
result0 = [series2 res indicate s r0 r];
...
if nargout == 0
    ...
    grid on
    legend(['Indicator'],'Stock1: Over bought','Stock1: Over sold',...
        'Location','NorthWest')
    ...
    for i = 1:nargout
        switch i
            ...
            case 2
                varargout{2} = r0;
            case 3
                varargout{3} = r; % return (pnl)
            case 4
                varargout{4} = sh; % sharpe ratio
            case 5
                varargout{5} = indicate; % indicator
            case 6
                varargout{6} = result0;
            otherwise
